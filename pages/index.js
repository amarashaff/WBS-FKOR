...
export default function WBS() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const d = await r.json()
      if (r.ok) { setSubmitted(d.kode); setForm(emptyForm); setStep(1); fetchPublic() }
      else showToast('Error: ' + (d.error || 'Gagal mengirim'))
  ...
  const LockScreen = () => (
    <div className="lock-screen">
      ...
      <div className="admin-form">
        <input type="password" placeholder="Masukkan sandi admin" value={loginPass}
    ...
    if (submitted) return (
      <div className="page-body">
        <div className="card" style={{textAlign:'center',padding:'2.5rem 1.5rem',maxWidth:480,margin:'0 auto'}}>
          ...
          <div style={{fontSize:13,color:'var(--text-muted)',marginBottom:'1.5rem',lineHeight:1.6}}>Simpan kode ini untuk memantau status pengaduan di halaman Daftar Pengaduan.</div>
  ...
  const AdminPanel = () => {
    ...
    useEffect(() => {
      if (tab==='pengaturan'&&!settingsOK) {
        fetch('/api/admin?type=settings',{headers:{'x-admin-password':ADMIN_PASS}})
          .then(r=>r.json())
          .then(d=>{ if(d.emailAdmin!==undefined)setEmailAdmin(d.emailAdmin); if(d.emailCC!==undefined)setEmailCC(d.emailCC); setSettingsOK(true) })
          .catch(()=>{})
      }
    }, [tab, settingsOK])
    ...
    const saveSettings = async () => {
      setSavingS(true)
      try {
        const r = await fetch('/api/admin?type=settings',{method:'POST',headers:{'Content-Type':'application/json','x-admin-password':ADMIN_PASS},body:JSON.stringify({emailAdmin,emailCC})})
      ...
      <>
        <div className="hero" style={{padding:'1.25rem 1.5rem 1rem'}}>
          ...
          <div style={{display:'flex',gap:4,marginTop:'1rem',borderBottom:'0.5px solid var(--border)'}}>
            {[['evaluasi','🔍 Evaluasi'],['export','📤 Export Data'],['pengaturan','⚙️ Pengaturan']].map(([k,l])=>(
            ...
            <>
              <div className="card">
                ...
                <div className="form-grid" style={{maxWidth:540}}>
                  <div className="form-group full"><label>Email admin utama</label><input type="email" value={emailAdmin} onChange={e=>setEmailAdmin(e.target.value)} placeholder="wbs@fkor.uns.ac.id"/></div>
                  <div className="form-group full"><label>Email CC (pisahkan koma)</label><input type="email" value={emailCC} onChange={e=>setEmailCC(e.target.value)} placeholder="dekan@fkor.uns.ac.id, wakil@fkor.uns.ac.id"/></div>
                </div>
                <div className="btn-row"><button className="btn btn-primary" onClick={saveSettings} disabled={savingS}>{savingS?'Menyimpan...':'Simpan perubahan'}</button></div>
              </div>
    ...
    <>
      ...
      <div className="app">
        <div className="sidebar">
          ...
          <div style={{padding:'6px 10px',fontSize:11,color:'var(--text-hint)',borderTop:'0.5px solid var(--border)',marginTop:4}}>WBS v2.2 · FKOR UNS</div>
        </div>
