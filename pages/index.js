...
function WBSLogo({ size = 32 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M32 13 L48 20.5 L48 34 C48 43 41 50.5 32 53 C23 50.5 16 43 16 34 L16 20.5 Z" fill="none" stroke="white" stroke-width="2" opacity="0.9"/>
      <rect x="19" y="27" width="10" height="13" rx="2" fill="white"/>
      <path d="M29 25 L43 19 L43 38 L29 33 Z" fill="white"/>
      <path d="M45 24 Q49 28.5 45 33" stroke="white" strokeWidth="2.5" strokeLinecap="round" fill="none"/>
      <path d="M48 21 Q54 28.5 48 36" stroke="white" strokeWidth="2" strokeLinecap="round" fill="none" opacity="0.6"/>
      <circle cx="46" cy="16" r="5" fill="#FAC775"/>
      <path d="M46 13 L46.8 15.4 L49.4 15.4 L47.3 16.9 L48.1 19.4 L46 17.9 L43.9 19.4 L44.7 16.9 L42.6 15.4 L45.2 15.4 Z" fill="#633806"/>
    </svg>
  )
}
...
export default function WBS() {
  ...
  const fetchPublic = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(d)) {
        ...
        setStats({
          total: all.length,
          menunggu: all.filter(x=>x.status==='Menunggu').length,
          proses: all.filter(x=>x.status==='Dalam Proses').length,
  ...
  const navTo = (p) => {
    setPage(p)
    if (p === 'daftar') setTimeout(fetchPublic, 50)
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setStep(1)
        fetchPublic()
        if (d.mode === 'demo') showToast('Pengaduan terkirim (mode demo — konfigurasi Supabase untuk produksi)')
      } else {
        showToast('Error: ' + (d.error || 'Gagal mengirim'))
      }
    } catch(e) {
  ...
  const doExport = async (periode) => {
    ...
    } catch(e) { showToast('Gagal mengunduh: ' + e.message) }
  }
      ...
      <div className="page-body">
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem',marginBottom:'1rem'}}>
          <div className="card">
            ...
            {pubData.length === 0 && <div style={{fontSize:13,color:'var(--text-hint)',padding:'0.75rem 0',textAlign:'center'}}>Belum ada pengaduan tercatat</div>}
          </div>
          <div className="card">
        ...
        <div className="card" style={{background:'#EAF3DE',borderColor:'#C0DD97'}}>
          <div style={{display:'flex',alignItems:'center',gap:'1rem',flexWrap:'wrap'}}>
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:13,color:'#3B6D11'}}>Gunakan kode pelacak yang ditampilkan setelah pengiriman untuk memantau perkembangan pengaduan.</div>
            </div>
    ...
    if (submitted) return (
      <div className="page-body">
        <div className="card" style={{textAlign:'center',padding:'2.5rem 1.5rem',maxWidth:480,margin:'0 auto'}}>
          ...
          <div style={{fontSize:22,fontWeight:500,fontFamily:'monospace',background:'#E6F1FB',color:'#0C447C',display:'inline-block',padding:'10px 24px',borderRadius:8,marginBottom:16,letterSpacing:'0.05em'}}>{submitted}</div>
          <div style={{fontSize:13,color:'var(--text-muted)',marginBottom:'1.5rem',lineHeight:1.6}}>Simpan kode ini. Gunakan kode untuk memantau status pengaduan di halaman Daftar Pengaduan.</div>
        ...
        <div className="page-body">
          ...
          {step === 1 && (
            <div>
              <div className="info-box blue">
                ...
                <p>Email Anda bersifat <strong>rahasia sepenuhnya</strong>. Hanya digunakan sebagai kunci akses laporan dan tidak akan disebarkan kepada siapapun.</p>
              </div>
              <div className="card">
                ...
                <div className="form-grid">
                  <div className="form-group">
                    <label>Email *</label>
                    <input type="email" placeholder="email@domain.com" value={lf.email} onChange={e=>set('email',e.target.value)} />
                    <span className="form-hint">Kunci akses — tidak akan disebarkan</span>
                  </div>
                  <div className="form-group">
                    <label>Nama (opsional — bisa anonim)</label>
                    <input type="text" placeholder="Nama atau inisial" value={lf.nama} onChange={e=>set('nama',e.target.value)} />
                  </div>
                  <div className="form-group">
                    <label>Status / Peran *</label>
                    <select value={lf.status_pelapor} onChange={e=>set('status_pelapor',e.target.value)}>
                  ...
                  <div className="form-group">
                    ...
                    <input type="text" placeholder="Nomor identitas akademik" value={lf.nim_nip} onChange={e=>set('nim_nip',e.target.value)} />
                  </div>
                </div>
                <div className="btn-row">
                  <button className="btn btn-primary" onClick={()=>{
                    if(!lf.email||!lf.status_pelapor){showToast('Email dan status wajib diisi');return}
                    setForm(lf); setStep(2)
                  }}>Lanjut →</button>
                </div>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="form-grid">
                <div className="form-group">
                  ...
                  <select value={lf.kategori} onChange={e=>set('kategori',e.target.value)}>
                    <option value="">-- Pilih --</option>
                ...
                <div className="form-group">
                  ...
                  <select value={lf.unit} onChange={e=>set('unit',e.target.value)}>
                    <option value="">-- Pilih --</option>
                ...
                <div className="form-group">
                  ...
                  <input type="text" placeholder="Misal: Gedung A, Lapangan Basket" value={lf.lokasi} onChange={e=>set('lokasi',e.target.value)} />
                </div>
                <div className="form-group">
                  <label>Tanggal kejadian *</label>
                  <input type="date" value={lf.tanggal_kejadian} onChange={e=>set('tanggal_kejadian',e.target.value)} />
                </div>
                <div className="form-group">
                  <label>Tingkat prioritas *</label>
                  <select value={lf.prioritas} onChange={e=>set('prioritas',e.target.value)}>
                    <option value="">-- Pilih --</option>
                ...
                <div className="form-group">
                  ...
                  <select value={lf.sudah_lapor} onChange={e=>set('sudah_lapor',e.target.value)}>
                    {['Belum','Sudah (internal)','Sudah (eksternal)'].map(o=><option key={o}>{o}</option>)}
                  </select>
                </div>
                <div className="form-group full">
                  <label>Judul pengaduan *</label>
                  <input type="text" placeholder="Ringkasan singkat pengaduan" value={lf.judul} onChange={e=>set('judul',e.target.value)} />
                </div>
                <div className="form-group full">
                  <label>Uraian lengkap *</label>
                  <textarea placeholder="Jelaskan: apa yang terjadi, kapan, siapa terlibat, dan dampaknya..." value={lf.uraian} onChange={e=>set('uraian',e.target.value)} />
                </div>
                <div className="form-group full">
                  <label>Harapan / usulan tindak lanjut</label>
                  <textarea style={{minHeight:60}} placeholder="Apa yang Anda harapkan dari laporan ini?" value={lf.harapan} onChange={e=>set('harapan',e.target.value)} />
                </div>
                <div className="form-group full">
                  <label>Lampiran (opsional)</label>
                  <input type="text" placeholder="Link Google Drive atau URL bukti pendukung" value={lf.lampiran} onChange={e=>set('lampiran',e.target.value)} />
                </div>
              </div>
              <div className="btn-row">
                ...
                <button className="btn btn-primary" onClick={()=>{
                  ...
                  for(const[k,l] of req){ if(!lf[k]){showToast(`${l} wajib diisi`);return} }
                  setForm(lf); setStep(3)
                }}>Lanjut →</button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="card">
              ...
              <div style={{fontSize:13,lineHeight:2,color:'var(--text-muted)',marginBottom:'1rem'}}>
  ...
  const AdminPanel = () => {
    ...
    const [emailAdmin, setEmailAdmin] = useState('wbs@fkor.uns.ac.id')
    ...
    useEffect(() => {
      if (tab === 'pengaturan' && !settingsLoaded) {
        fetch('/api/admin?type=settings', { headers: { 'x-admin-password': ADMIN_PASS } })
          .then(r => r.json())
          .then(d => {
            if (d.emailAdmin !== undefined) setEmailAdmin(d.emailAdmin)
            if (d.emailCC !== undefined) setEmailCC(d.emailCC)
    ...
    const saveSettings = async () => {
      ...
      try {
        const r = await fetch('/api/admin?type=settings', {
          method: 'POST',
          headers: { 'Content-Type':'application/json', 'x-admin-password': ADMIN_PASS },
          body: JSON.stringify({ emailAdmin, emailCC })
        })
        if (r.ok) showToast('Pengaturan berhasil disimpan ✓')
      ...
      <>
        <div className="hero" style={{padding:'1.25rem 1.5rem 1rem'}}>
          ...
          <div style={{display:'flex',gap:4,marginTop:'1rem',borderBottom:'0.5px solid var(--border)'}}>
            {tabs.map(([key,label]) => (
              <button key={key} onClick={()=>setTab(key)} style={{padding:'6px 14px',border:'none',background:'none',fontFamily:'inherit',fontSize:13,cursor:'pointer',color:tab===key?'#0C447C':'var(--text-muted)',borderBottom:tab===key?'2px solid #378ADD':'2px solid transparent',fontWeight:tab===key?500:400}}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div className="page-body">
          ...
          {tab === 'evaluasi' && (
            adminLoading
              ? <div style={{textAlign:'center',padding:'2rem',color:'var(--text-hint)',fontSize:13}}>Memuat data...</div>
              : items.length === 0
                ? <div className="card" style={{textAlign:'center',padding:'2rem',color:'var(--text-hint)',fontSize:13}}>Belum ada pengaduan</div>
                : items.map((d,i) => (
                  <div key={d.id} className="card">
                    <div style={{display:'flex',alignItems:'flex-start',gap:'1rem',flexWrap:'wrap'}}>
                      <div style={{flex:1,minWidth:200}}>
                        ...
                        {d.uraian && <div style={{fontSize:12,color:'var(--text-muted)',marginTop:6,lineHeight:1.5,background:'var(--bg-surface)',padding:'6px 8px',borderRadius:6}}>{d.uraian.length>140?d.uraian.slice(0,140)+'...':d.uraian}</div>}
                      </div>
                      <div style={{minWidth:260,display:'flex',flexDirection:'column',gap:6}}>
                        ...
                        <textarea style={{minHeight:55,fontSize:12}} placeholder="Tambahkan catatan tindak lanjut..."
                          value={d._catatan}
                          onChange={e=>{const n=[...items];n[i]={...n[i],_catatan:e.target.value};setItems(n)}}
                        />
                        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                          <select style={{fontSize:12,padding:'5px 8px',width:'auto'}}
                            value={d._status}
                            onChange={e=>{const n=[...items];n[i]={...n[i],_status:e.target.value};setItems(n)}}
                          >
                            {['Menunggu','Dalam Proses','Selesai','Ditolak'].map(s=><option key={s}>{s}</option>)}
                          </select>
            ...
            <>
              ...
              <div className="card" style={{background:'#EAF3DE',borderColor:'#C0DD97'}}>
                ...
                <div style={{fontSize:13,color:'#3B6D11',lineHeight:1.9}}>
                  Sheet 1 — Pengaduan: semua kolom termasuk email pelapor (rahasia)<br/>
                  Sheet 2 — Ringkasan: statistik total, per status, dan tanggal export
                </div>
              </div>
            </>
          )}
            ...
            <>
              <div className="card">
                ...
                <div className="form-grid" style={{maxWidth:540}}>
                  <div className="form-group full">
                    ...
                    <input type="email" value={emailAdmin} onChange={e=>setEmailAdmin(e.target.value)} placeholder="wbs@fkor.uns.ac.id" />
                  </div>
                  <div className="form-group full">
                    <label>Email CC (pisahkan koma)</label>
                    <input type="email" value={emailCC} onChange={e=>setEmailCC(e.target.value)} placeholder="dekan@fkor.uns.ac.id, wakil@fkor.uns.ac.id" />
                  </div>
                </div>
                <div className="btn-row">
              ...
              <div className="card">
                ...
                <div className="info-box amber">
                  ...
                  <p>Untuk mengubah sandi admin, edit environment variable <strong>ADMIN_PASSWORD</strong> di dashboard Vercel, lalu redeploy. Ini memastikan sandi tersimpan dengan aman.</p>
                </div>
                <div style={{fontSize:13,color:'var(--text-muted)',lineHeight:1.7}}>
                  1. Buka dashboard Vercel → pilih project WBS<br/>
                  2. Settings → Environment Variables → cari <code style={{background:'var(--bg-surface)',padding:'1px 5px',borderRadius:4}}>ADMIN_PASSWORD</code><br/>
                  3. Edit nilainya → Save → Redeploy
                </div>
              </div>
              <div className="card">
                <div className="card-title">Kebijakan privasi & keamanan</div>
                <div style={{fontSize:13,color:'var(--text-muted)',lineHeight:2}}>
                  {['Email pelapor tidak tampil di antarmuka publik','Identitas hanya bisa diakses admin dengan sandi','Seluruh data tersimpan di database terenkripsi (Supabase)','Endpoint admin dilindungi header x-admin-password','Log akses tercatat untuk audit trail'].map(t=>(
                    <div key={t}>🔒 {t}</div>
                  ))}
                </div>
              </div>
  ...
  const navItems = [
    ['beranda','🏠','Beranda'],
    ['laporan','📝','Buat Pengaduan'],
    ['daftar','📋','Daftar Pengaduan'],
    ['admin','⚙️','Pengaturan'],
  ]
    ...
    <>
      ...
      <div className="app">
        <div className="sidebar">
          <div className="sidebar-brand">
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
              ...
              <div>
                ...
                <div className="brand-name">WBS Keolahragaan</div>
              </div>
            </div>
          </div>
          ...
          <div style={{padding:'6px 10px',fontSize:11,color:'var(--text-hint)',borderTop:'0.5px solid var(--border)',marginTop:4}}>WBS v2.1 · FKOR UNS</div>
        </div>
        <div className="main">
          {pages[page]}
        </div>
