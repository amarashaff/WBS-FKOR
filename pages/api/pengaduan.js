...
export default async function handler(req, res) {
  ...
  if (db) {
    try {
      let q = db.from('pengaduan').select(PUBLIC).order('created_at', { ascending: false })
      ...
      console.error('[WBS pengaduan]', e.message)
    }
  }
  ...
  if (search) {
    ...
    data = data.filter(d => d.judul?.toLowerCase().includes(q) || d.kode?.toLowerCase().includes(q) || d.kategori?.toLowerCase().includes(q))
  }
