...
export default async function handler(req, res) {
  ...
  const body = req.body || {}
  const { email, nama, status_pelapor, nim_nip, kategori, unit, lokasi,
          tanggal_kejadian, prioritas, sudah_lapor, judul, uraian, harapan, lampiran } = body
  ...
  if (isSupabaseReady()) {
    try {
      ...
      const { data, error } = await sb.from('pengaduan').insert([{
        kode:             entry.kode,
        email:            entry.email,
        nama:             entry.nama,
      ...
      // Jangan return error — fallback ke memory
      console.error('[WBS] Supabase insert failed, falling back:', e.message)
    }
  }

  // ── Mode demo/fallback: in-memory ────────────────────────────
  global._wbsData.unshift(entry)
