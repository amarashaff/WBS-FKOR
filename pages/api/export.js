...
export default async function handler(req, res) {
  ...
  if (isSupabaseReady()) {
    try {
      ...
      let q = sb.from('pengaduan').select('*').order('created_at', { ascending: false })
      if (periode === 'minggu') {
        ...
        q = q.gte('created_at', new Date(now.getFullYear(), 0, 1).toISOString())
      }
      const { data: rows, error } = await q
      if (error) return res.status(500).json({ error: error.message })
      data = rows
    } catch (e) {
      console.error('[WBS] export Supabase error:', e.message)
      data = [...(global._wbsData || [])]
    }
  } else {
    data = [...(global._wbsData || [])]
  }
